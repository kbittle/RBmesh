...
impl BmNetworkRoutingHdr {
    // Constructor
    pub fn new() -> BmNetworkRoutingHdr {
        BmNetworkRoutingHdr { 
            src: None,
            next_hop: None,
            orig: None,
    ...
    pub const fn with_ttl(self, new_ttl: u8) -> Self {
        self.info.with_ttl(new_ttl);
        self
    }

    pub const fn with_ack_required(self, new_with_required_ack: bool) -> Self {
        self.info.with_required_ack(new_with_required_ack);
        self
    }
...
impl BmNetworkPacket {
    // Constructor
    pub fn new(new_packet_type: BmPacketTypes, orig: NetworkId, dest: NetworkId) -> BmNetworkPacket {
        BmNetworkPacket {
            packet_type: new_packet_type,
            routing_hdr: BmNetworkRoutingHdr::new()
                .with_src(orig)
                .with_orig(orig)
                .with_dest(dest),
            payload: None,
    ...
    // Closest thing we can have to a overloaded constructor
    pub fn new_with_payload(new_packet_type: BmPacketTypes, orig: NetworkId, dest: NetworkId, new_payload: Option<BmNetworkPacketPayload>) -> Self {
        BmNetworkPacket {
            packet_type: new_packet_type,
            routing_hdr: BmNetworkRoutingHdr::new()
                .with_src(orig)
    ...
    pub fn from(length: usize, buffer: &mut [u8]) -> Option<BmNetworkPacket> {
        ...
        if length > BM_PACKET_HDR_SIZE {
            ...
            payload_vec.extend_from_slice(buffer[18..].try_into().unwrap()).unwrap();
        ...
        Some(BmNetworkPacket {
            packet_type: BmPacketTypes::from_bits(buffer[0]),
                routing_hdr: BmNetworkRoutingHdr {
                    dest: Some(u32::from_be_bytes(buffer[1..4].try_into().unwrap())),
                    src: Some(u32::from_be_bytes(buffer[5..8].try_into().unwrap())),
                    next_hop: Some(u32::from_be_bytes(buffer[9..12].try_into().unwrap())),
                    orig: Some(u32::from_be_bytes(buffer[13..16].try_into().unwrap())),
    ...
    pub fn to_bytes(&mut self) -> Option<BmNetworkPacketPayload> {
        ...
        if out_buffer.extend_from_slice(&self.routing_hdr.dest.unwrap_or(0).to_le_bytes()).is_err() { return None; }
        if out_buffer.extend_from_slice(&self.routing_hdr.src.unwrap_or(0).to_le_bytes()).is_err() { return None; }
        if out_buffer.extend_from_slice(&self.routing_hdr.next_hop.unwrap_or(0).to_le_bytes()).is_err() { return None; }
        if out_buffer.extend_from_slice(&self.routing_hdr.orig.unwrap_or(0).to_le_bytes()).is_err() { return None; }
